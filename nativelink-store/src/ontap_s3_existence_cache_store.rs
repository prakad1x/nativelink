    ...
    NowFn: Fn() -> I + Send + Sync + Unpin + Clone + 'static,
{
    ...
    // New method to run the sync process
    ...
    // Modified sync_cache to support incremental updates
    ...
    pub async fn new(spec: &OntapS3ExistenceCacheSpec, now_fn: NowFn) -> Result<Arc<Self>, Error> {
        let inner_spec = match &*spec.backend {
            StoreSpec::ontap_s3_store(s3_spec) => s3_spec,
            _ => {
                return Err(make_err!(
        ...
        tokio::spawn(async move {
            ...
            let mut interval = interval(Duration::from_secs(
                cache_clone.sync_interval_seconds as u64,
            ));
            loop {
                interval.tick().await;
...
async fn create_s3_client(spec: &OntapS3Spec) -> Result<Client, Error> {
    // Use AWS default credentials provider which checks env vars
    ...
    NowFn: Fn() -> I + Send + Sync + Unpin + Clone + 'static,
{
    async fn has_with_results(
        ...
        // No longer calls check_sync() - sync happens in background only
